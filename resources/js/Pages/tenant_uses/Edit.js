...
const Edit = (props) => {
  ...
  const { tenantUse } = usePage().props  
 console.log(errors);
  const [form, setForm] = useState({
      name: tenantUse.name,
      account_number: tenantUse.account_number
    })
   
  const handleSubmit = e => {
      e.preventDefault()
   
      Inertia.post('/tenant-uses/'+tenantUse.id, {
          _method: 'put',
          name: form.name,
        ...
        <div>
            <Layout>
                ...
                <div className="header">
                    <h1 className="header-text">Edit Tenant Use</h1>
                </div>
                     
               <div className="md:grid md:grid-cols-4 md:gap-6">     
                            
               <div className="md:col-span-1">
                <InertiaLink href="/tenant-uses" className="p-2 mt-8 text-lg font-bold text-white bg-black hover:bg-gray-700">
                  Back to Tenant Uses
                  </InertiaLink>
              </div>
              {/* right side */}
              <div className="mt-5 md:mt-0 md:col-span-2">
               ...
               <form>
                {errors[0] && <div className="text-sm text-red-500"><b>{errors[0]}</b></div>}
